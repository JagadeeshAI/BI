...
def train_teacher_model(class_ranges, teacher_type, save_name, base_ckpt_path):
    """
    Train a teacher model on specified class ranges, starting from a fixed base checkpoint.
    
    Args:
        class_ranges: list of tuples [(start, end), ...]
        teacher_type: "good" or "bad"
        save_name: filename for saving the model
        base_ckpt_path: path to the shared starting checkpoint (0-49.pth)
    """
    print(f"\n🚀 Training {teacher_type.upper()} Teacher: {save_name}")
    print(f"📚 Training on class ranges: {class_ranges}")
    ...
    # Always start from the same base checkpoint
    model = get_model(num_classes=num_classes, use_lora=False, pretrained=False)
    load_model_weights(model, base_ckpt_path)  # Load 0-49.pth
    model.to(device)
    ...
    from torch.utils.data import ConcatDataset
    combined_train_dataset = ConcatDataset([loader.dataset for loader in all_train_loaders])
    combined_val_dataset = ConcatDataset([loader.dataset for loader in all_val_loaders])
...
def main():
    ...
    teacher_configs = [
        {"good_ranges": [(10, 59)], "bad_ranges": [(0, 9), (60, 99)], "student_name": "student_10_59"},
        {"good_ranges": [(20, 69)], "bad_ranges": [(0, 19), (70, 99)], "student_name": "student_20_69"},
        {"good_ranges": [(30, 79)], "bad_ranges": [(0, 29), (80, 99)], "student_name": "student_30_79"},
        {"good_ranges": [(40, 89)], "bad_ranges": [(0, 39), (90, 99)], "student_name": "student_40_89"},
        {"good_ranges": [(50, 99)], "bad_ranges": [(0, 49)], "student_name": "student_50_99"},
    ]
    ...
    for config in teacher_configs:
        ...
        good_teacher_name = f"{student_name}_good_teacher"
        train_teacher_model(config["good_ranges"], "good", good_teacher_name, base_ckpt_path)

        bad_teacher_name = f"{student_name}_bad_teacher"
        train_teacher_model(config["bad_ranges"], "bad", bad_teacher_name, base_ckpt_path)
